...
class CircleLayer: CALayer {
    ...
    var progress: CGFloat = 0.0 {
        didSet{
            ...
            outsideRect = CGRect(x:originX,
                                 y:originY,
                                 width:outsideRectSize,
                                 height:outsideRectSize);
            setNeedsDisplay()
        }
    }
    ...
    override func draw(in ctx: CGContext) {
        ...
        let movedDistance = (outsideRect.size.width * 1 / 6) * abs(self.progress - 0.5) * 2
        let rectCenter = CGPoint(x:outsideRect.origin.x + outsideRect.size.width/2 ,
                                 y:outsideRect.origin.y + outsideRect.size.height/2)
        ...
        ctx.setLineDash(phase: 2.0, lengths: dash)
        ctx.strokePath()
        ...
        ctx.setLineDash(phase: 0.0, lengths:[2,3])
